...
bot.action(/slot_student_.+/, (ctx) => {
  const raw = ctx.callbackQuery && "data" in ctx.callbackQuery
...
bot.command("faq", (ctx) => ctx.reply("FAQ (здесь позже появятся вопросы и ответы)"));
...
bot.on("text", (ctx, next) => {
  ...
  switch (s.step) {
    ...
    case "patronymic":
      ...
      return ctx.reply("/student_birth_date\nДень рождения (формат 00.00.0000)");
